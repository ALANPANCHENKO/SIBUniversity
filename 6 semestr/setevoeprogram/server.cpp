...
int main() {
    ...
    for (;;) {
        ...
        // Отправляем обратное подтверждение "выполнено" клиенту
        const char *response = "выполнено";
        sendto(sockMain, response, strlen(response), 0, (struct sockaddr *)&clientAddr, length);

        printf("SERVER: Отправлено клиенту: %s\n\n", response);
    }

    // Закрытие сокета (Note: This part is unreachable in the current implementation)
    close(sockMain);
